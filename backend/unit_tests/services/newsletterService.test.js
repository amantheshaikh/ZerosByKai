...
describe('newsletterService.js', () => {
    beforeEach(() => {
        ...
        const methods = ['from', 'select', 'eq', 'in', 'order', 'limit', 'insert', 'update', 'single', 'maybeSingle', 'rpc'];
    ...
    describe('pickAndPublishIdeas()', () => {
        ...
        it('should create a batch and publish ideas if no batch exists', async () => {
            ...
            const mockIdeas = Array.from({ length: 10 }, (_, i) => ({ id: i + 1, status: 'scheduled', week_published: mockWeekStart }));
            ...
            // 3. rpc called to pick & publish ideas
            supabaseAdmin.rpc.mockResolvedValueOnce({
                data: mockIdeas,
                error: null
            });

            // 4. update batch metrics (Chain 5: .from.update.eq)
            supabaseAdmin.eq
                .mockReturnValueOnce(supabaseAdmin) // Chain 1
                .mockResolvedValueOnce({ error: null }); // TERMINAL
        ...
        it('should throw an error if fewer than 10 approved ideas are found', async () => {
            ...
            await expect(pickAndPublishIdeas()).rejects.toThrow(/which is less than 10/);
        });
        ...
        it('should handle update errors when publishing ideas', async () => {
            ...
            await expect(pickAndPublishIdeas()).rejects.toThrow('Failed to atomically schedule ideas: RPC Failed');
        });
    });
});
